...
class CategoryRepository {
  ...
  Stream<List<Category>> watchCategoriesByType(CategoryType type) =>
      _dao.watchCategoriesByType(type);
  ...
  Future<void> createCategory(CategoriesCompanion category) async {
    ...
    final currentCategories = await _dao
        .watchCategoriesByType(category.type.value)
        .first;
    ...
    await _dao.insertCategory(
      category.copyWith(name: Value(name), sortOrder: Value(maxSortOrder + 1)),
    );
  }
